...
class DisplayTests(unittest.TestCase):
    ...
    def test_auto_uses_authenticated_gpu_display_with_no_rfb_listener(self):
        with tempfile.TemporaryDirectory() as directory, \
            ...
            self.assertNotIn('-rfbunixpath', command)
            self.assertTrue(popen.call_args.kwargs['start_new_session'])
            self.assertTrue(run.call_args_list[0].kwargs['input'].startswith('add :0 MIT-MAGIC-COOKIE-1 '))
            self.assertTrue(run.call_args_list[1].kwargs['input'].startswith('add :12 MIT-MAGIC-COOKIE-1 '))
    ...
    def test_forced_xvnc_exercises_cpu_only_ci_without_auto_render_device(self):
        with tempfile.TemporaryDirectory() as directory, \
             ...
             patch.object(display.subprocess, 'Popen', side_effect=self.start_process) as popen, \
             patch.object(display.subprocess, 'run'), \
